    ...
    <div className="w-full h-full bg-[#080808] flex flex-col rounded-3xl overflow-hidden border border-white/5 text-white relative">
      ...
      <div className="flex bg-black/40 border-b border-white/5 shrink-0">
         ...
         <div className="flex-1 flex items-center px-8 gap-8">
            {['Dashboard', 'Warehouse', 'Orders', 'Reports'].map(tab => (
              <div 
                 key={tab} 
                 onClick={() => setActiveTab(tab)}
                 className={cn("text-[10px] font-black uppercase tracking-widest cursor-pointer hover:text-orange-500 transition-colors", activeTab === tab ? "text-orange-500" : "text-white/40")}
              >
                 {tab}
              </div>
            ))}
      ...
      <div className="flex-1 p-8 overflow-y-auto">
         {activeTab === 'Dashboard' ? (
           ...
           <div className="h-full flex flex-col items-center justify-center opacity-40">
              <Box size={48} className="text-orange-500 mb-6" />
              <h4 className="text-xl font-black uppercase tracking-widest mb-2">System Interfacing</h4>
              <p className="text-[10px] uppercase font-bold tracking-[0.2em] text-text-dimmed">Connecting to regional nodes...</p>
           </div>
         )}
      </div>
    </div>
